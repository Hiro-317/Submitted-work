...
EffectFire::EffectFire(const ParameterLoad& parameter, EFFECT_NAME name, const Vector3& local, const Transform* follow,
	...
	: EffectBase(parameter)
{
	...
	info.trans.LoadEffect("Fire");

	info.trans.angle = Vector3();
	info.trans.scale = GetParameterToVector3("Fire", "Scale");
}